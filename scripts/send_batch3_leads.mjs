...
async function main() {
  ...
  let targetFileName = process.env.LEADS_FILE || "LEADS_BATCH_3.csv";
  let dryRun = false;
  let limit = Infinity;
  let startIndex = 0;
  ...
  console.log(`\n============================================================`);
  console.log(`BREVO OUTREACH CAMPAIGN: BATCH 3`);
  ...
  console.log(`Mode:                ${dryRun ? "🔍 DRY RUN (Simulated, no emails sent)" : "🚀 LIVE SENDING VIA BREVO"}`);
  console.log(`============================================================\n`);
  ...
  for (let i = 0; i < targetLeads.length; i++) {
    ...
    try {
      ...
      failCount++;
      console.error(`[${actualIndex}/${leads.length}] ERROR: Failed for ${lead.business_name} (${recipientEmail}):`, err.message);
    }
  }

  console.log("\n============================================");
  console.log(`Campaign Summary (${targetFileName})`);
  console.log(`Status:              ${dryRun ? "DRY RUN COMPLETE" : "CAMPAIGN COMPLETE"}`);
